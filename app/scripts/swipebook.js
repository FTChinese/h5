...
Swipe.prototype = {
  ...
  slide: function(index, duration, turnpage) {
    ...
    if (this.type==="course"){
  	...
  	if (lastmove>=0 && lastmove<n) {
  		thepos=firstmovedirection*this.width+"px";
  			setTimeout(function(){
  			$(jq).eq(lastmove).css({'-webkit-transform':'translate3d(' + thepos + ',0,0)','-moz-transform':'translate3d(' + thepos + ',0,0)','-ms-Transform':'translateX(' + thepos + ')','-o-Transform':'translateX(' + thepos + ')'}).show();			
  		},0);
  	}
  ...
  onTouchStart: function(e) {
    ...
    _eventHistory.push({ x: e.touches[0].pageX, y: e.touches[0].pageY, t: Number( new Date() ) });

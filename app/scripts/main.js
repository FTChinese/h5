...
function openbook() {
    ...
    $("#allcontent .slide").each(function(i){
		...
		if ($(this).attr('title') && ($(this).css('background-image') !== 'none' || $(this).attr('thumbnail') || $(this).attr('data-fixed-bg'))) {
            ...
            $("#imageRailInner").append('<div class="rail'+borderColor+'" n1target = '+i+' onclick="jumptoarticle('+i+',0)"><div class="railImage"><div class=railImageTable><div class=railImageCell style="background-image:' + thumbImg + ';"></div></div></div><div class="railText">'+$(this).attr('title') + '</div></div>');
        } else if ($(this).find("img.headshot").length>0) {
			$("#imageRailInner").append('<div class="rail'+borderColor+'" n1target = '+i+' onclick="jumptoarticle('+i+',0)"><div class="railImage"><div class=railImageTable><div class=railImageCell style="background-image:url(' + $(this).find("img.headshot").attr("src") + ');"></div></div></div><div class="railText">'+$(this).find(".headline").eq(0).html()+'</div></div>');
            ...
            $("#imageRailInner").append('<div class="rail'+borderColor+'" n1target = '+i+' onclick="jumptoarticle('+i+',0)"><div class="railImage"><div class=railImageTable><div class=railImageCell style="background-image:url(' + $(this).find('video').attr("poster") + ');"></div></div></div><div class="railText">'+$(this).attr('title')+'</div></div>');
            headshotImg='<img src="'+$(this).find("video.headshot").attr("poster")+'" class=rightimage>';
        } else if ($(this).find(".secStart").length>0) {
            $("#imageRailInner").append('<div class="rail'+borderColor+'" n1target = '+i+' onclick="jumptoarticle('+i+',0)"><div class="railImage"><div class=railImageTable><div class=railImageCell style="font-size:25px;background:#333;color:white;font-weight:bold;">'+$(this).find(".secStart").eq(0).html()+'</div></div></div><div class="railText">'+$(this).find(".secStart").eq(0).html()+'</div></div>');                //headshotImg='<img src="'+$(this).find("video.headshot").attr("poster")+'" class=rightimage>';
        }
